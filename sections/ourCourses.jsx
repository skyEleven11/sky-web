  ...
  useEffect(()=>{
    if(courses[1]){
      ...
      courses.map((illustration)=> {
        const DynamicIllustration =  dynamic(() => import(`../components/illustrations/${illustration.ILUSTRACION}`,{ssr: false}))
        illArray.push(<DynamicIllustration customClass={styles.courseIllustrarionSize}/>)
      })
      ...
      <div className={styles.timeline}>
        {courses &&
          ...
          courses.map((course, i) => (
            <>
              <div className={`${styles.individualCourseCont} ${ i%2 === 0 ? styles.left : `${styles.right} flex-row-reverse`}`} key={i + (new Date).getTime()}>
                <div className={styles.content}>
                  <h6 className="small-headers white-blue-text">{course.NOMBRE}</h6>
